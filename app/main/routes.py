...
from app.main.forms import EditProfileForm, PostForm, EmptyForm, MessageForm
...
def index():
    ...
    # use paginate to divide posts, this is done on the server side
...
def user(username):
    ...
    form = EmptyForm()
    return render_template("main/user.html", user=user, posts=posts.items, next_url=next_url, prev_url=prev_url, form=form)
...
def messages():
    ...
    return render_template('main/view_message.html', messages=messages.items, next_url=next_url, prev_url=prev_url)
